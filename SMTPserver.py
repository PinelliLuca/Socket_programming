...
def process_mail_command(conn, data, msg):
    if data is not None:
        ...
        print('process mail command: cmd="%s", param="%s"'%(cmd, param))
        if cmd == 'QUIT': process_quit(conn, msg, msg='Goodbye')
        if cmd == 'HELO': send_response(conn, '250', msg='Session started')
        if cmd == 'MAIL': process_envelope(conn, msg, cmd, param)
        if cmd == 'RCPT': process_envelope(conn, msg, cmd, param)
        if cmd == 'DATA': send_response(conn, '354', msg='Senda data, end with "."')
        return cmd
...
def add_msg_body(data, msg):
    ...
    msg['rfc822'] += data
    m=re.search('\r\n.\r\n', data)
    print('adding data to body: "%s", result is %r' % (data, bool(m)))
    return bool(m)
