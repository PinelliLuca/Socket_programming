...
def is_valid_cidr(cidr):
    ...
    parts = cidr.split('/') #controllo di avere il formato <ip_addr>/<mask_len>
    if len(parts) != 2:
        return False
    ip, mask = parts
...
def main():
    ...
    ip, mask = cidr.split('/') #separa la stringa in due quando incontra il carattere '/', quindi mi separa le due parti dell'indirizzo, "ip_address" e "netmask_len"
    print("IP: ",ip)
    print("Mask ",mask)
    data = {
        "netid": ip,
